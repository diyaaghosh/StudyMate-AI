...
tabs = st.tabs([" Chat"," Summary"," Quiz"])
...
with tabs[1]:
    ...
    topic = st.text_input("Enter topic or PDF name",key="summary_topic")
    if st.button("Generate Summary",key="summary_btn"):
        with st.spinner( "Creating summary..."):
            result = rag.summarize_document( topic, top_k=20)
...
with tabs[2]:
    ...
    if "quiz" in st.session_state:
        ...
        if not quiz["quiz"]:
            st.error("No quiz content found in documents :(.")
        else:
            st.subheader(f"{quiz['topic']} Quiz")
